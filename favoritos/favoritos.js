...
if (btnLogout) {
  btnLogout.addEventListener("click", async () => {
    try {
      ...
      window.location.replace("../login/loghi.html");
    } catch (error) {
...
onAuthStateChanged(auth, async (user) => {
  if (!user) {
    window.location.href = "../login/loghi.html";
    return;
  }

  if (nomeUsuario) {
    nomeUsuario.innerText = user.displayName || "Meu Perfil";
  }

  if (fotoUsuario && user.photoURL) {
